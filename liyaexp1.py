...
def gui():
        ...
        try:
            while True:
                ...
                if communi.in_waiting > 0:
                    ...
                    px.alert(text=line)  # Corrected this line
                    a = px.confirm('Enter option', buttons=['correct', 'wrong'])
                if a == 'correct':
                    break
                final_entry.delete(1.0, tk.END)  # Clear the final_entry Text widget
    ...
    collect_data_button = tk.Button(root, text="Collect Data", command=lambda: collect_data(), font=("Arial", 20))  # Updated button text
